...
'is_exemplar': substage.is_exemplar,
            ...
            if include_tools:
                stage_data['tool_count'] = Tool.query.filter_by(stage_id=stage.id).count()
    ...
    try:
        ...
        for tool in pagination.items:
            tool_data = {
                ...
                'substage': {
                    'id': tool.substage.id,
                    'name': tool.substage.name
                } if tool.substage else None
            }
...
def search_tools():
    ...
    try:
        ...
        aggregations = {
            ...
            ).join(Tool).group_by(LifecycleStage.id).all(),
            
            'source_types': db.session.query(
                Tool.source_type,
                func.count(Tool.id).label('count')
            ).filter(Tool.source_type.isnot(None)).group_by(Tool.source_type).all(),
            
            'scopes': db.session.query(
                Tool.scope,
                func.count(Tool.id).label('count')
            ).filter(Tool.scope.isnot(None)).group_by(Tool.scope).all()
        }
...
def recommend_tools():
    ...
    try:
        ...
        if source_stage_id:
            source_tools = Tool.query.filter_by(
        ...
        if target_stage_id:
            target_tools = Tool.query.filter_by(
        ...
        if interaction_type == 'data_transfer':
            # Recommend ETL tools
            etl_tools = Tool.query.filter(
                Tool.tool_type.ilike('%ETL%')
            ).limit(3).all()
        ...
        return jsonify({
            'status': 'error',
            'message': 'Failed to recommend tools'
        }), 500
...
def get_network_data():
    ...
    try:
        ...
        for conn in connections:
            if (not stage_ids or 
                (str(conn.from_stage_id) in stage_ids.split(',') and 
                 str(conn.to_stage_id) in stage_ids.split(','))):
                edges.append({
                    'id': f'conn_{conn.id}',
        ...
        return jsonify({
            ...
            'data': {
                ...
                'statistics': {
                    ...
                    'stage_count': len([n for n in nodes if n['type'] == 'stage']),
                    'tool_count': len([n for n in nodes if n['type'] == 'tool']),
                    'interaction_count': len([n for n in nodes if n['type'] == 'interaction'])
                }
            }
        }), 200
    ...
    try:
        ...
        stats = {
            ...
            'interactions_by_complexity': db.session.query(
                ...
                func.count(Interaction.id).label('count')
            ).group_by(Interaction.complexity).all(),
            
            'top_tool_providers': db.session.query(
                Tool.provider,
                func.count(Tool.id).label('count')
            ).filter(Tool.provider.isnot(None)).group_by(Tool.provider).order_by(
                func.count(Tool.id).desc()
            ).limit(10).all()
        }
        ...
        formatted_stats = {
            ...
            'distributions': {
                'tools_by_source_type': [
                    {'type': s[0], 'count': s[1]}
                    for s in stats['tools_by_source_type']
                ],
                'tools_by_scope': [
                    {'scope': s[0], 'count': s[1]}
                    for s in stats['tools_by_scope']
                ],
