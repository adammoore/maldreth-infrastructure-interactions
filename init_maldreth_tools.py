...
class MaLDReTHDataInitializer:
    ...
    def run(self) -> bool:
        ...
        try:
            logger.info(
                f"Starting MaLDReTH initialization from {self.file_type} file: {self.file_path}"
            )
                ...
                try:
                    ...
                    for header_row in [0, 5, 6]:
                        try:
                            ...
                            if (
                                len(df.columns) > 3
                            ):  # Valid data should have multiple columns
                                break
                        except:
                            continue
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Create tool
                    tool = Tool(
                        name=tool_name, category_id=category.id, stage_id=stage.id
                    )

                    # Add optional fields
                    if columns.get("description"):
                        tool.description = self._clean_value(
                            row[columns["description"]]
                        )
    ...
    def _get_or_create_stage(self, name: str) -> Optional[Stage]:
        ...
        if not stage:
            # Create new stage
            stage = Stage(name=name, description=self._get_stage_description(name))
    ...
    def _get_or_create_category(
        self, name: str, description: str, stage: Stage
    ...
    def _process_examples(
        self, examples: str, category: ToolCategory, stage: Stage
    ) -> None:
        ...
        for tool_name in tool_names:
            ...
            if not existing_tool:
                tool = Tool(name=tool_name, category_id=category.id, stage_id=stage.id)
                db.session.add(tool)
                self.stats["tools_created"] += 1
                logger.debug(f"Created tool: {tool_name}")
        ...
        for from_name, to_name, conn_type in self.STANDARD_CONNECTIONS:
            try:
                ...
                if not from_stage or not to_stage:
                    logger.warning(
                        f"Stages not found for connection: {from_name} -> {to_name}"
                    )
                    continue
                ...
                connection = Connection(
                    from_stage_id=from_stage.id, to_stage_id=to_stage.id, type=conn_type
                )
    ...
    def _get_stage_description(self, stage_name: str) -> str:
        ...
        return descriptions.get(
            stage_name.upper(),
            f"Stage for {stage_name.lower()} in the research data lifecycle",
        )
...
Examples:
  ...
  python init_maldreth_tools.py --file data/tools.xlsx --clear
    ...
    parser.add_argument(
        "--clear", action="store_true", help="Clear existing data before initialization"
    )
