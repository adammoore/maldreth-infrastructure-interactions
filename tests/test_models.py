...
class TestStageModel:
    ...
    def test_create_stage(self, app):
        ...
        with app.app_context():
            stage = Stage(name="Test Stage", description="Test Description")
    ...
    def test_stage_relationships(self, app):
        ...
        with app.app_context():
            ...
            # Add category
            category = ToolCategory(category="Test Category", stage_id=stage.id)
...
class TestToolCategoryModel:
    ...
    def test_category_tools_relationship(self, app):
        ...
        with app.app_context():
            ...
            # Add tools
            tool1 = Tool(name="Tool 1", category_id=category.id, stage_id=stage.id)
            tool2 = Tool(name="Tool 2", category_id=category.id, stage_id=stage.id)
...
class TestToolModel:
    ...
    def test_tool_optional_fields(self, app):
        ...
        with app.app_context():
            stage = Stage(name="Test Stage")
            category = ToolCategory(category="Test Category", stage=stage)
            db.session.add_all([stage, category])
            db.session.commit()

            # Create tool with only required fields
            tool = Tool(name="Minimal Tool", category_id=category.id, stage_id=stage.id)
            db.session.add(tool)
            db.session.commit()
    ...
    def test_tool_cascade_delete(self, app):
        ...
        with app.app_context():
            ...
            tool = Tool(name="Test Tool", category_id=category.id, stage_id=stage.id)
            db.session.add(tool)
            db.session.commit()
...
class TestConnectionModel:
    ...
    def test_create_connection(self, app):
        ...
        with app.app_context():
            ...
            connection = Connection(
                from_stage_id=stage1.id, to_stage_id=stage2.id, type="solid"
            )
    ...
    def test_connection_repr(self, app):
        ...
        with app.app_context():
            ...
            connection = Connection(from_stage=stage1, to_stage=stage2)
    ...
    def test_connection_types(self, app):
        ...
        with app.app_context():
            ...
            dashed_conn = Connection(
                from_stage_id=stage2.id, to_stage_id=stage1.id, type="dashed"
            )
    ...
    def test_connection_self_reference(self, app):
        ...
        with app.app_context():
            ...
            connection = Connection(
                from_stage_id=stage.id, to_stage_id=stage.id, type="solid"
            )
            db.session.add(connection)
            db.session.commit()
    ...
    def test_connection_cascade_delete(self, app):
        ...
        with app.app_context():
            ...
            connection = Connection(from_stage_id=stage1.id, to_stage_id=stage2.id)
            db.session.add(connection)
            db.session.commit()
...
class TestComplexQueries:
    ...
    def test_get_tools_by_stage(self, app):
        ...
        with app.app_context():
            ...
            for i in range(3):
                tool = Tool(name=f"Tool {i}", category_id=cat1.id, stage_id=stage1.id)
                db.session.add(tool)

            # Add tools to stage 2
            tool = Tool(name="Other Tool", category_id=cat2.id, stage_id=stage2.id)
            db.session.add(tool)
            db.session.commit()
    ...
    def test_get_connected_stages(self, app):
        ...
        with app.app_context():
            ...
            for i in range(4):
                connection = Connection(
                    from_stage_id=stages[i].id, to_stage_id=stages[(i + 1) % 4].id
                )
                db.session.add(connection)
            db.session.commit()

            # Get connections from Stage 0
            connections = Connection.query.filter_by(from_stage_id=stages[0].id).all()
    ...
    def test_search_tools(self, app):
        ...
        with app.app_context():
            ...
            results = Tool.query.filter(
                db.or_(
                    Tool.name.ilike("%project%"), Tool.description.ilike("%project%")
                )
            ).all()

            assert len(results) == 2
            assert all(
                "project" in r.name.lower() or "project" in r.description.lower()
                for r in results
            )
