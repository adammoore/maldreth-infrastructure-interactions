                ...
                try:
                    ...
                    if stage_name_upper in stages:
                        ...
                        for db_stage_name, db_stage_id in stages.items():
                            if stage_name_upper in db_stage_name or db_stage_name in stage_name_upper:
                                stage_id = db_stage_id
                                break
                    
                    if not stage_id:
                        logger.warning(f"Row {idx}: Stage '{stage_name}' not found in database, skipping")
                        continue
...
def main():
    ...
    if not csv_file:
        ...
        for f in csv_files:
            if any(keyword in f.name.lower() for keyword in ['tool', 'lifecycle', 'research', 'maldreth', 'landscape']):
                csv_file = str(f)
    ...
    if not csv_file:
        ...
        logger.error("Please ensure you have a CSV file with tool data in the current directory")
        sys.exit(1)
