    ...
    with engine.connect() as conn:
        ...
        if "lifecycle_stages" in inspector.get_table_names():
            ...
            if "maldreth_description" not in columns:
                ...
                conn.execute(
                    text(
                        "ALTER TABLE lifecycle_stages ADD COLUMN maldreth_description TEXT"
                    )
                )
                conn.commit()
            ...
            if "color_code" not in columns:
                ...
                conn.execute(
                    text(
                        "ALTER TABLE lifecycle_stages ADD COLUMN color_code VARCHAR(7) DEFAULT '#007bff'"
                    )
                )
                conn.commit()
            ...
            if "icon" not in columns:
                ...
                conn.execute(
                    text(
                        "ALTER TABLE lifecycle_stages ADD COLUMN icon VARCHAR(50) DEFAULT 'bi-circle'"
                    )
                )
                conn.commit()
            ...
            if "is_active" not in columns:
                ...
                conn.execute(
                    text(
                        "ALTER TABLE lifecycle_stages ADD COLUMN is_active BOOLEAN DEFAULT TRUE"
                    )
                )
                conn.commit()
            ...
            if "created_at" not in columns:
                ...
                conn.execute(
                    text(
                        "ALTER TABLE lifecycle_stages ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
                    )
                )
                conn.commit()
            ...
            if "updated_at" not in columns:
                ...
                conn.execute(
                    text("ALTER TABLE lifecycle_stages ADD COLUMN updated_at TIMESTAMP")
                )
                conn.commit()
