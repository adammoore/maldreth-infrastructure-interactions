...
def inspect_database():
    ...
    with engine.connect() as conn:
        for table in tables:
            ...
            if table in ['lifecycle_stages', 'tool_categories', 'tools', 'interactions']:
                print(f"\nSample data from {table}:")
                if table == 'lifecycle_stages':
                    result = conn.execute(text(f"SELECT id, name, \"order\" FROM {table} ORDER BY \"order\" LIMIT 5"))
                elif table == 'interactions':
                    result = conn.execute(text(f"SELECT id, interaction_type, source_infrastructure, target_infrastructure FROM {table} LIMIT 5"))
                else:
