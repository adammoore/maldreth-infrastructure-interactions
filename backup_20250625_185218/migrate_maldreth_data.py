            ...
            try:
                ...
                if not stage:
                    ...
                    for stage_key, stage_obj in stages.items():
                        if (
                            stage_name_upper in stage_key
                            or stage_key in stage_name_upper
                        ):
                            stage = stage_obj
                            break
                ...
                if not stage:
                    logger.warning(
                        f"Row {idx}: Stage '{stage_name}' not found in MaLDReTH stages, skipping"
                    )
                    continue
                ...
                tool_category = ToolCategory.query.filter_by(
                    stage_id=stage.id, name=category_name
                ).first()
                ...
                if not tool_category:
                    ...
                    tool_category = ToolCategory(
                        stage_id=stage.id, name=category_name, description=description
                    )
                    db.session.add(tool_category)
                    db.session.flush()  # Ensure category has an ID
                    logger.info(
                        f"Created category: {category_name} for stage: {stage.name}"
                    )
                    ...
                    for tool_name in tools:
                        if tool_name and tool_name != "":  # Skip empty tool names
                            ...
                            existing_tool = Tool.query.filter_by(
                                name=tool_name, stage_id=stage.id
                            ).first()
                ...
                if csv_files:
                    ...
                    for f in csv_files:
                        if any(
                            ...
                            for keyword in ["tool", "lifecycle", "research", "maldreth"]
                        ):
                            csv_file = str(f)
                    ...
                    logger.error(
                        "No CSV file found. Please ensure one of the following files exists:"
                    )
