...
class LifecycleStage(db.Model):
    ...
    tool_categories = db.relationship(
        "ToolCategory", backref="stage", lazy="dynamic", cascade="all, delete-orphan"
    )
    tools = db.relationship(
        "Tool", backref="stage", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class ToolCategory(db.Model):
    ...
    # Relationships
    tools = db.relationship(
        "Tool", backref="category", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Tool(db.Model):
    ...
    # Foreign keys
    stage_id = db.Column(
        db.Integer, db.ForeignKey("lifecycle_stages.id"), nullable=True
    )
    substage_id = db.Column(
        db.Integer, db.ForeignKey("lifecycle_substages.id"), nullable=True
    )
    category_id = db.Column(
        db.Integer, db.ForeignKey("tool_categories.id"), nullable=True
    )
...
class StageConnection(db.Model):
    ...
    connection_type = db.Column(
        db.String(50), default="normal"
    )  # 'normal', 'alternative', 'feedback'
    description = db.Column(db.Text)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "from_stage_id != to_stage_id", name="check_no_self_connection"
        ),
    )
...
class Interaction(db.Model):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_interaction_infrastructures",
    ...
    def __repr__(self):
        return (
            f"<Interaction {self.source_infrastructure}->{self.target_infrastructure}>"
        )
